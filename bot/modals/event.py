        ...
        try:
            ...
            if not self.new_roster:
                ...
                try:
                    ...
                    day_change = RosterExtended.did_day_change(old_date, self.bot.rosters[self.channel_id].date,
                                                               self.config["raids"]["timezone"])
                    event_change = RosterExtended.did_trial_change(old_event, self.bot.rosters[self.channel_id].event)
                    if not day_change:
                    ...
                    if self.sort_channels or self.change_name:
                        new_name = RosterExtended.generate_channel_name(formatted_date, event,
                                                                             self.config['raids']['timezone'])
                        await self.channel.edit(name=new_name)

                    if day_change or event_change:
                        name = RosterExtended.create_pingable_role_name(trial=event,
                                                                        date=formatted_date,
                ...
                try:
                    ...
                    self.bot.rosters[self.channel_id] = RosterExtended.event_factory(leader, event, formatted_date,
                                                                                     memo, role.id)

                    embed = EmbedFactory.create_new_event_roster(event=self.bot.rosters[self.channel_id].event, date=self.bot.rosters[self.channel_id].date,
                    ...
                    logging.error(f"Event Roster Creation Channel And Embed Error: {str(e)}")
                    return
            else:
                await interaction.response.send_message(
            ...
            logging.error(f"Event Roster Error During Channel Create and Embed: {str(e)}")
            await interaction.response.send_message(
                f"{Utilities.format_error(self.user_language, self.localization['Unreachable'])}")
            return
