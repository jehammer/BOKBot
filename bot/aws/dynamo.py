...
class Dynamo:
    ...
    def get(self, query):
        ...
        try:
            ...
            logging.error(
                f"Dynamo Get Item Error Table: %s, error: %s: %s",
                self.table_name,
    ...
    def put(self, data):
        ...
        try:
            ...
            logging.error(
                f"Dynamo Put Item Error Table: %s, error: %s: %s",
                self.table_name,
                e.response["Error"]["Code"],
                e.response["Error"]["Message"],
            )
    ...
    def delete(self, query):
        ...
        try:
            ...
            logging.error(
                f"Dynamo Delete Item Error Table: %s, error: %s: %s",
                self.table_name,
                e.response["Error"]["Code"],
                e.response["Error"]["Message"],
            )
    ...
    def update(self, query, data):
        ...
        try:
            ...
            expression = update[
                :-2
            ]  # NOTE: Need to remove trailing space and comma included in for-loop
            ...
            logging.error(
                f"Dynamo Update Item Error Table: %s, error: %s: %s",
                self.table.name,
                e.response["Error"]["Code"],
                e.response["Error"]["Message"],
            )
    ...
    def scan_get_all(self):
        ...
        try:
            ...
            logging.error(
                f"Dynamo Scan Get All Items Error Table: %s, error: %s: %s",
                self.table.name,
                e.response["Error"]["Code"],
                e.response["Error"]["Message"],
            )
